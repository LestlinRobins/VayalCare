...
import { Mic, TestTube, DollarSign, Calendar, Users, Cloud, BookOpen, ShoppingCart, Camera, Calculator, Newspaper, FileText, MapPin, SprayCan, Map, Wheat, LifeBuoy, PlayCircle } from "lucide-react";
...
const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  useEffect(() => {
    document.title = language === "ml" ? "ഹോം | കാർഷിക ഡാഷ്ബോർഡ്" : "Home | Farm Dashboard";
  }, [language]);
  ...
  const handleMicClick = () => {
    ...
    if (!rec) {
      toast({
        ...
        description: language === "ml" ? "ഈ ബ്രൗസറിൽ മൈക്ക് പിന്തുണയില്ല." : "Microphone support is not available in this browser."
      });
      return;
    }
    ...
    rec.onerror = (event: any) => {
      ...
      toast({
        ...
        description: language === "ml" ? "വോയ്സ് തിരിച്ചറിയാൻ കഴിഞ്ഞില്ല" : "Voice recognition failed. Please try again."
      });
    };
    rec.onend = () => {
      setListening(false);
  ...
  return <div className="pb-20 bg-background min-h-screen transition-colors duration-300">
      {/* Greeting Banner */}
      <div className="relative h-56 rounded-b-3xl overflow-hidden transition-colors duration-300" style={{
      backgroundImage: "url('/lovable-uploads/afdc9b1b-83d4-4fb1-be61-4f53c9ff0ad1.png')",
      ...
      backgroundPosition: "center"
        ...
        <img src="/lovable-uploads/60f927d7-a6b0-4944-bf34-9a7a5394d552.png" alt={language === "ml" ? "കാലാവസ്ഥ ഐകൺ" : "Weather icon - partly cloudy"} className="absolute bottom-2 right-2 z-10 w-24 h-24 object-contain-center" loading="eager" />
      </div>

      {/* Voice Assistant Button */}
      <Button variant={listening || isProcessing ? "secondary" : "default"} className="fixed top-4 right-4 z-20 h-14 w-14 rounded-full shadow-lg transition-colors duration-300" onClick={handleMicClick} aria-pressed={listening} disabled={isProcessing} title={listening ? language === "ml" ? "കേൾക്കുന്നു…" : "Listening…" : isProcessing ? language === "ml" ? "പ്രോസസ്സിംഗ്..." : "Processing..." : language === "ml" ? "വോയ്സ് അസിസ്റ്റന്റ്" : "Voice assistant"}>
        <Mic className={`h-6 w-6 ${listening ? "animate-pulse text-red-500" : isProcessing ? "animate-spin text-blue-500" : ""}`} />
      </Button>

      {/* Live Speech Display */}
      {(listening || interimText) && <div className="fixed top-20 right-4 z-20 bg-background/90 backdrop-blur-sm border border-border rounded-lg p-3 max-w-xs shadow-lg">
          <div className="text-xs text-muted-foreground mb-1">
            {language === "ml" ? "കേൾക്കുന്നു..." : "Listening..."}
          </div>
          <div className="text-sm text-foreground font-medium">
            {interimText || (language === "ml" ? "സംസാരിക്കുക..." : "Speak now...")}
          </div>
        </div>}
      ...
      <div className="p-4 space-y-6">
        ...
        <div>
          <div className="grid grid-cols-3 gap-3">
            ...
            return <button key={action.id} onClick={() => toast({
              title: action.label,
              description: language === "ml" ? "പ്ലേസ്‌ഹോൾഡർ - നിങ്ങൾ പിന്നീട് പുതുക്കാം" : "Placeholder - you can update later"
                  ...
                  <div className="w-12 h-12 text-primary flex items-center justify-center mx-auto mb-2">
                    {"image" in action && action.image ? <img src={action.image as string} alt={`${action.label} icon`} className="h-10 w-10 md:h-12 md:w-12 object-contain-center" loading="lazy" /> : <Icon className="h-6 w-6" />}
                  </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            containerType: "inline-size"
              ...
              <div className="marquee marquee-right gap-2 md:gap-3 items-stretch" style={{
              animationDuration: "30s"
            }}>
                {[...featureContent, ...featureContent].map((item, idx) => <div key={`top-${item.id}-${idx}`} className="shrink-0" style={{
                width: idx % 2 === 0 ? "40cqw" : "60cqw"
                    ...
                    <Card className="overflow-hidden hover:shadow-lg transition-shadow rounded-md h-full">
                      <div className="bg-muted relative h-28 md:h-32">
                        <img src={item.image} alt={item.title} className="w-full h-full object-cover-center" loading="lazy" />
                      </div>
            ...
            containerType: "inline-size"
              ...
              <div className="marquee marquee-left gap-2 md:gap-3 items-stretch" style={{
              animationDuration: "30s"
            }}>
                {[...featureContent, ...featureContent].map((item, idx) => <div key={`bottom-${item.id}-${idx}`} className="shrink-0" style={{
                width: idx % 2 === 0 ? "40cqw" : "60cqw"
              }}>
                    <Card className="overflow-hidden hover:shadow-lg transition-shadow rounded-md h-full">
                      <div className="bg-muted relative h-28 md:h-32">
            ...
            return <Card key={feature.id} className="farm-mgmt-bg cursor-pointer hover:shadow-lg transition-all duration-300 border-0 bg-muted rounded-[10%] overflow-hidden" onClick={() => onFeatureClick(feature.id)}>
                  <CardContent className="p-4 text-center m-0">
                    <div className="w-24 h-24 text-primary flex items-center justify-center mx-auto mb-1 transition-colors duration-300">
                      {feature.id === "diagnose" ? <img src="/lovable-uploads/fb9c0289-77d0-4856-9028-76b4f16989dd.png" alt={`${feature.title} icon`} className="h-20 w-20 md:h-28 md:w-28 object-contain-center" loading="lazy" /> : feature.id === "market" ? <img src="/lovable-uploads/00c4bc0c-067f-4b9e-bdb0-816ecd25ad76.png" alt={`${feature.title} icon`} className="h-24 w-24 md:h-32 md:w-32 object-contain-center" loading="lazy" /> : feature.id === "twin" ? <img src="/lovable-uploads/f9697d94-aedf-499f-93d5-7bcfe3319ac7.png" alt={`${feature.title} icon`} className="h-24 w-24 md:h-32 md:w-32 object-contain-center" loading="lazy" /> : "image" in feature && feature.image ? <img src={feature.image as string} alt={`${feature.title} icon`} className="h-24 w-24 md:h-32 md:w-32 object-contain-center" loading="lazy" /> : <Icon className="h-24 w-24 md:h-32 md:w-32" />}
                    </div>
