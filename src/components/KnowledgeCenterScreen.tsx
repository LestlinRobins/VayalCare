...
import { BookOpen, Users, ArrowLeft, ShoppingCart, Calculator, Newspaper, FileText } from 'lucide-react';
  ...
  return <div className="pb-20 bg-background min-h-screen transition-colors duration-300">
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          return <Card key={tool.id} className="cursor-pointer hover:shadow-lg transition-all duration-300 border-0 bg-muted rounded-[10%] overflow-hidden" onClick={() => onFeatureClick?.(tool.id)}>
                <CardContent className="p-4 text-center m-0">
                  <div className="w-24 h-24 text-primary flex items-center justify-center mx-auto mb-3 transition-colors duration-300">
                    <img src={tool.image} alt={`${tool.title} icon`} className="h-24 w-24 object-contain-center" loading="lazy" />
                  </div>
                  <h3 className="font-medium text-foreground text-sm transition-colors duration-300">{tool.title}</h3>
                </CardContent>
              </Card>;
        })}
        </div>
